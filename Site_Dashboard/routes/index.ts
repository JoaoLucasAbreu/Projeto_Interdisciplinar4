...
class IndexRoute {
	public static async index(req: app.Request, res: app.Response) {
		...
		if (!u) {
			...
			let passagens: any[];
			await app.sql.connect(async (sql) => {
				passagens = await sql.query("SELECT voo.destino, tp_passagem.preco, tp_passagem.tipoPassagem, tp_passagem.hSaida, tp_passagem.duracao, passagem.companhia, passagem.dataPesquisa FROM passagem INNER JOIN voo ON passagem.idVoo = voo.idVoo INNER JOIN tp_passagem ON passagem.idPassagem = tp_passagem.idPassagem WHERE tp_passagem.tipoPassagem = ? ", ["MENOR"]) as [] ; 
			});
